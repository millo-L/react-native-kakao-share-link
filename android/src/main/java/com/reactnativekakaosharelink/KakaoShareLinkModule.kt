...
class KakaoShareLinkModule(private val reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
    ...
    private fun sendWithTemplate(template: DefaultTemplate, promise: Promise) {
        ...
        if (LinkClient.instance.isKakaoLinkAvailable(this.reactContext)) {
            this.reactContext.currentActivity?.let {
                LinkClient.instance.defaultTemplate(it, template, serverCallbackArgs) { linkResult, error ->
                    if (error != null) {
                        ...
                        linkResult?.intent?.let { it1 -> ContextCompat.startActivity(it, it1, null) }
    ...
    @ReactMethod
    fun sendCommerce(dict: ReadableMap, promise: Promise) {
    ...
    @ReactMethod
    fun sendList(dict: ReadableMap, promise: Promise) {
    ...
    @ReactMethod
    fun sendFeed(dict: ReadableMap, promise: Promise) {
    ...
    @ReactMethod
    fun sendLocation(dict: ReadableMap, promise: Promise) {
    ...
    @ReactMethod
    fun sendText(dict: ReadableMap, promise: Promise) {
